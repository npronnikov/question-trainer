...
- Все изменяющие запросы защищаются стандартным CSRF-токеном Spring Security. Frontend получает токен из разрешённого same-origin cookie/header-контракта и отправляет его в заголовке; CORS с `*` удаляется.
...
### 9.4 Жизненный цикл оценки

AI-проверка выполняется асинхронно, чтобы HTTP-запрос не зависел от времени ответа модели:

1. `POST /api/practice/attempts` валидирует четыре поля, создаёт попытку и возвращает `attemptId` со статусом `EVALUATING`.
2. Backend запускает ограниченную по времени assessment job и сохраняет её результат транзакционно.
3. Frontend получает завершение через SSE либо повторный `GET`; бизнес-результат одинаков в обоих каналах.
4. Состояние меняется только `EVALUATING → PASSED | NEEDS_REVISION | UNVERIFIED`; повторная доставка результата идемпотентна.
5. Timeout, перезапуск процесса или невалидный JSON завершают попытку как `UNVERIFIED`, а не оставляют её бесконечно активной.

### 9.5 Строгий ответ модели
...
Старые chat-таблицы привязываются к seeded admin, поскольку это существующие личные рабочие диалоги владельца инсталляции. Общий импортированный curriculum и старые сгенерированные ситуации получают зарезервированного системного владельца без возможности входа. Миграции не теряют существующие записи.
...
- `/api/practice/attempts` — четыре поля и запуск асинхронной оценки;
- `/api/practice/attempts/{id}` — результат и история исправлений;
- `/api/practice/attempts/{id}/events` — SSE завершения оценки;
